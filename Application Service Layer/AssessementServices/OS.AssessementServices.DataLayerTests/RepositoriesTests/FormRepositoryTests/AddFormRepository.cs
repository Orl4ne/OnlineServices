...
namespace OS.AssessementServices.DataLayerTests.RepositoriesTests.FormRepositoryTests
{
	[TestClass]
	public class AddFormTests
	{
		[TestMethod]
		public void ThrowsExceptionWhenANonExistingFormQuestionIsProvided()
		{
			...
			{
				// Arrange - Act
				IFormRepository repository = new FormRepository(memoryContext);
		...
		[TestMethod]
		public void ShouldInsertInDbWhenValidFormIsSupplied()
		{
			var options = new DbContextOptionsBuilder<EvaluationContext>()
				.UseInMemoryDatabase(databaseName: MethodBase.GetCurrentMethod().Name)
				.Options;
			...
			using (var memoryContext = new EvaluationContext(options))
			{
				...
				// Act
				var result = formRepository.GetAll().FirstOrDefault(f => f.Name.French == "Formulaire d'évaluation journalier");

				// Assert
				Assert.AreEqual("Formulaire d'évaluation journalier", result.Name.French);
			}
		}
	}
}
