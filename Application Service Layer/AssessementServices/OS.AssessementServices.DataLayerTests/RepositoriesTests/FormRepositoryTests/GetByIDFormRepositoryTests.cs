...
{
	[TestClass]
	public class GetByIDFormRepositoryTests
	{
		[TestMethod]
		public void AdddNewFormThenRetrieveItIsNotNull()
		{
			...
			using (var memoryContext = new EvaluationContext(options))
			{
				...
				// Act
				var result = formRepository.GetById(formAdded1.Id);

				// Assert
				Assert.IsNotNull(result);
				Assert.AreEqual("Daily evaluation form", result.Name.English);
			}
		}
	}
}
