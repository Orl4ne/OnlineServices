...
namespace OS.AssessementServices.DataLayerTests.RepositoriesTests.QuestionRepositoryTest
{
	[TestClass]
	public class GetAllQuestionRepositoryTest
	{
		[TestMethod]
		public void GetAllAddThreeQuestionReturnsCorrectNumberOfQuestion()
		{
			...
			using (var memoryContext = new EvaluationContext(options))
			{
				...
				var count = questionRepository.GetAll().Count();
				...
				{
					//System.Console.WriteLine($"Id : {question.Id} - Form : {question.FormId.Name.English} - Position : {question.Position} - Type : {question.Type}");
					System.Console.WriteLine($"\t{question.Libelle.English}");
					System.Console.WriteLine($"\t{question.Libelle.French}");
					System.Console.WriteLine($"\t{question.Libelle.Dutch}");
				}
			}
		}
	}
