...
{
	[TestClass]
	public class AddQuestionRepositoryTest
	{
		[TestMethod]

		public void AddQuestionAddNewQuestionReturnCount()
		{
			...
			using (var memoryContext = new EvaluationContext(options))
			{
				...
				// Assert
				Assert.IsNotNull(questionAdded1);
				Assert.IsNotNull(questionAdded2);
				Assert.IsNotNull(questionAdded3);

				Assert.AreEqual(5, memoryContext.Questions.Count());
			}
		}
	}
}
