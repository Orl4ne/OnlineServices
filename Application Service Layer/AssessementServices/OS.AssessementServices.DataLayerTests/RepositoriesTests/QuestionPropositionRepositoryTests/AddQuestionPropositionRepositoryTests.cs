...
namespace OS.AssessementServices.DataLayerTests.RepositoriesTests.QuestionPropositionRepositoryTests
{
	[TestClass]
    public class AddQuestionPropositionRepositoryTests
    {
        [TestMethod]
        ...
        [TestMethod()]
        public void ShouldInsertInDbWhenValidQuestionPropositionIsSupplied()
        {
            var options = new DbContextOptionsBuilder<EvaluationContext>()
                .UseInMemoryDatabase(databaseName: MethodBase.GetCurrentMethod().Name)
                .Options;
            ...
            using (var memoryContext = new EvaluationContext(options))
            {
                ...
                //ASSERT
                Assert.IsNotNull(questionPropositionAdded1);
                Assert.IsNotNull(questionPropositionAdded2);
                Assert.IsNotNull(questionPropositionAdded3);

                Assert.AreEqual("medium", questionPropositionAdded2.Libelle.English);
            }
        }
    }
}
