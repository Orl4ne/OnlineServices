...
namespace OS.AssessementServices.DataLayerTests.RepositoriesTests.QuestionPropositionRepositoryTests
{
	...
	{
		[TestMethod]
		public void RemoveQuestionPropositionAddNewQuestionPropostionThenRemoveItReturnsTrue()
		{
			...
			using (var memoryContext = new EvaluationContext(options))
			{
				...
				// ACT
				// Remove by entity
				var result1 = questionPropositionRepository.Remove(questionPropositionAdded1);
				// Remove by id
				var result2 = questionPropositionRepository.Remove(questionPropositionAdded2.Id);
				memoryContext.SaveChanges();

				// Assert
				Assert.IsTrue(result1);
				Assert.IsTrue(result2);
				Assert.AreEqual(4, questionPropositionRepository.GetAll().Count());
			}
		}
	}
}
