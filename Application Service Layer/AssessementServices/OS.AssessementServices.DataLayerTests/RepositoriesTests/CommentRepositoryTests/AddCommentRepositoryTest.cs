...
namespace OS.AssessementServices.DataLayerTests.RepositoriesTests.CommentRepositoryTests
{
    [TestClass]
    public class AddCommentRepositoryTest
    {
        [TestMethod]
        ...
        [TestMethod]
        public void ShouldInsertInDbWhenValidCommentIsSupplied()
        {
            var options = new DbContextOptionsBuilder<EvaluationContext>()
                .UseInMemoryDatabase(databaseName: MethodBase.GetCurrentMethod().Name)
                .Options;
            ...
            using (var memoryContext = new EvaluationContext(options))
            {
                ...
                var result = commentRepository.GetAll().FirstOrDefault(c => c.Content == "ceci est un commentaire");
